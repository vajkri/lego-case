...
### Phase 1: Foundation
**Goal**: The project runs as a verified static export with correct architecture boundaries, typed data for all 14 topics, and dev conventions that any developer can follow immediately.
...
**Success Criteria** (what must be TRUE):
  ...
  2. All 14 topic entries exist in TypeScript data files with typed coordinates, slide headings, and body content sourced from `proposal-content.md`
...
**Goal**: A presenter can navigate through all 14 topics using only the keyboard, see each topic's slides in a full-screen overlay, step through sub-slides, and return to the map — with accessibility correct from the start.
**Depends on**: Phase 1
...
**Success Criteria** (what must be TRUE):
  1. Pressing ArrowRight or Space from the map advances to the next topic's slides; pressing ArrowLeft goes back; pressing Escape closes the current slide and returns to the map
  ...
  3. On a topic with 3 sub-slides, pressing ArrowRight steps through all 3 in sequence, then closes the overlay and returns to the map on the final advance
  4. When the slide overlay opens, focus is trapped inside it; when it closes, focus returns to the city node button that triggered it — verifiable with a keyboard-only walkthrough
  5. A screen reader announces the current topic name and sub-slide number each time they change, without the user taking any extra action
**Plans**: TBD

### Phase 3: Map and Car Animation
**Goal**: The LEGO world map is visible as the root view with all 14 named city nodes, and the red LEGO car travels smoothly between cities on the GPU compositor thread without dropped frames.
**Depends on**: Phase 2
**Requirements**: MAP-01, MAP-02, MAP-03, MAP-04, CAR-01, CAR-02, CAR-03, CAR-04
**Success Criteria** (what must be TRUE):
  1. The root view shows an SVG world map with all 14 city nodes labeled, each rendered as a focusable button accessible by keyboard tab order and announced by a screen reader
  2. Pressing advance on the map triggers the red LEGO car to animate along a path from the current city to the next city before the slide overlay opens
  3. Cities the presenter has already visited display a visually distinct state from unvisited cities, and a progress indicator shows the current position in the 14-topic journey
  4. Car travel animation runs at 60fps on a standard laptop — no visible jank during city-to-city transitions — verified by watching Chrome DevTools Performance panel during travel
**Plans**: TBD

### Phase 4: Content and Polish
**Goal**: All 14 topic slides are fully authored and visually complete with LEGO-inspired design tokens, the production build is clean, and the presentation is ready to give.
**Depends on**: Phase 3
...
**Success Criteria** (what must be TRUE):
  ...
  3. A run-through of all 14 topics from first city to last produces no broken slides, placeholder content, missing data, or visual regressions
**Plans**: TBD
